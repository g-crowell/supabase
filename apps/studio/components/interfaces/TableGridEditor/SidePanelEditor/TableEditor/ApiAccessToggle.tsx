...
interface ApiAccessToggleProps {
  ...
  onChange?: (privileges: ApiPrivilegesByRole) => void
  onInitialLoad?: (privileges: ApiPrivilegesByRole) => void
}

const createDefaultRolePrivileges = (): ApiPrivilegesByRole => ({
...
export const ApiAccessToggle = ({
  ...
  const derivedPrivileges: ApiPrivilegesByRole =
    tableFields.apiPrivileges ?? apiAccessData?.privileges ?? defaultPrivileges

  const emptyPrivileges: ApiPrivilegesByRole = useMemo(() => ({ anon: [], authenticated: [] }), [])
  ...
  const [savedPrivileges, setSavedPrivileges] = useState<ApiPrivilegesByRole>()
  ...
  const handlePrivilegesChange = (role: ApiAccessRole) => (values: string[]) => {
    if (!isSchemaExposed) return
    const updatedPrivileges: ApiPrivilegesByRole = {
      ...effectivePrivileges,
